...
$(document).ready(function () {
    ...
    $("#contact-submit").click(function () {
        var url = "https://1m0lviq5dd.execute-api.us-east-1.amazonaws.com/default/verify_captcha_leonfattakhov_web";
        var message = $("#messageInput").val();
        var email = $("#emailAddress").val();
        var data = {"message": message, "email": email, "gcap": grecaptcha.getResponse(myGCap)};
        $.ajax({
            type: "POST",
            url: url,
            data: data,
            success: (s)=>alert("success", s),
    ...
    $('.nav-link, .custom-navbrand').click(
        function () {
            $('.navbar-collapse').collapse('hide');
